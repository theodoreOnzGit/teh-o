...
///
///
/// now using rayon in parallel,
/// for 200 batches of particles
#[test]
pub fn test_5_multiple_batches(){
    ...
    //
    dbg!(&simulation_vector.len());

    // with rayon, I can use only par_iter functions
    // which means I cannot have mutable references to self
    //
    ...
    // let's do 200 batches 
    let number_of_batches = 200;

    #[inline]
    fn single_batch_calc(
        mut simulation_vector: 
        Vec<( MockTestMonoenergeticParticle, u64, CollisionTally)>,
        u235_total_xs: MicroscopicCrossSection,
        u235_scatter_xs: MicroscopicCrossSection,
        ...
        -> Vec<( MockTestMonoenergeticParticle, u64, CollisionTally)> {
            ...
            let sim_vector_finished: Vec<(
                ...
                u64,
                CollisionTally)> = simulation_vector.par_iter_mut().map(
                    |(neutron_ref, prn_seed, collision_tally)|{ 
                        // let's first do a random walk, and check the interaction
                        neutron_ref.random_walk_travel(prn_seed, u235_macro_total_xs);
                        // check the interaction there
                        let mut interaction = MockTestMonoenergeticParticle::
                            scatter_or_absorption_rng(prn_seed, u235_total_xs, u235_scatter_xs);

                        // score the collision tally 
                        collision_tally._add_interaction_to_tally(interaction);

                        // if the interaction is scattering, then scatter isotropically
                        if interaction == NeutronInteraction::_Scatter {
                            ...
                            neutron_ref.isotropically_scatter(prn_seed);
                        } else if interaction == NeutronInteraction::_Absorption {
                            // if absorption, do not do anything 
                        }
            ...
            return sim_vector_finished;
        }

    let simulation_timer = SystemTime::now();
    // first batch
    let mut sim_vector_finished = single_batch_calc(
        simulation_vector, 
        u235_total_xs, 
        u235_scatter_xs, 
        u235_macro_total_xs);

    for _i in 0..(number_of_batches-1) {

        sim_vector_finished = single_batch_calc(
            sim_vector_finished, 
            u235_total_xs, 
            u235_scatter_xs, 
            u235_macro_total_xs);
    ...
    let time_per_particle = simulation_timer.elapsed().unwrap()/
        number_of_particles as u32;
    dbg!(simulation_timer.elapsed().unwrap());
    ...
    let scatterings: Vec<u64> = sim_vector_finished.par_iter().map(
        |(_neutron_ref, _prn_seed, collision_tally)|{ 
            collision_tally._scatter_count
        }).collect();
