...
#[test]
pub fn test4b_random_walk_infinite_medium_scattering_many_particle_iterator(){
...
///
/// now using rayon in parallel,
/// First thing is to make the references immutable
#[test]
pub fn test4a_random_walk_infinite_medium_scattering_many_particle_iterator_non_mutable(){
    ...
    for neutron_ref in neutron_vector.iter(){
        while interaction == NeutronInteraction::_Scatter {
            ...
            let mut neutron_clone = neutron_ref.clone();

            // if scattering, then we sample new distance 
            neutron_clone.random_walk_travel(&mut prn_seed, u235_macro_total_xs);
            ...
            if interaction == NeutronInteraction::_Scatter {
                // get new direction if it is scattering 
                // but I'll abstract this into a isotropic scattering function
                neutron_clone.isotropically_scatter(
                    &mut prn_seed);
            } else if interaction == NeutronInteraction::_Absorption {
                // print particle 1 statistics, and delete it
                //dbg!(&neutron_ref);
